C
      SUBROUTINE MST5D(LSP,MV,EVAL,WORK,EVEC,N,M,ESP,ILL)
C
...
C
      REAL*8 LSP(N,M),MV(N),EVAL(N),WORK(N),EVEC(N,N)
      CALL MST2D(LSP,MV,EVEC,N,M)
      ILL=1
      CALL HOQRSD(EVEC,N,N,EVAL,WORK,ESP,ILL)
